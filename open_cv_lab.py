...
def remove_background_combined(image):
    otsu_result, otsu_mask = remove_background_otsu(image)
    adaptive_result, adaptive_mask = remove_background_adaptive(image)
    canny_result, canny_mask = remove_background_canny(image)
    ...
    combined_mask_rgb = cv2.merge([combined_mask, combined_mask, combined_mask])
    
    result = cv2.bitwise_and(image, combined_mask_rgb)
...
def standardize_size(img, target_size=(400, 600)):
    standardized = np.zeros((target_size[1], target_size[0], 3), dtype=np.uint8)
